...
A mounted project whose `vault_id` equals the live workspace vault's is a
**self-alias**: it is excluded from the vault registry entirely (the live
vault is the sole answerer for its own ID — no route, no read-only copy, no
staleness), its bridge endpoints resolve to the local `.marmot` directory
instead of the Warren copy, and it can never be editable or materialized.
`@<local-vault-id>/…` references resolve against the live in-memory graph at
the engine layer, never through the registry.
...
| **Static Analysis Indexer** | Automated code-to-graph indexing (`internal/indexer/`). Pluggable `Indexer` interface with three implementations: Go AST indexer (full `go/ast` analysis — packages, functions, methods, types, interfaces with `contains`/`imports`/`calls`/`extends`/`implements` edges), TypeScript regex-based indexer (classes, interfaces, functions, arrow functions, type aliases, methods with string/comment masking), and generic file indexer (30+ extensions, module-level nodes with best-effort import detection). `Registry` maps extensions to indexers. `Runner` orchestrates directory walks with `.gitignore` support, incremental indexing via source hash comparison, optional CRUD classification, batch embedding, and `RunResult` reporting. Integrated into CLI: `marmot index <path>` for static analysis, `marmot index <path> --incremental` for change-only re-indexing. |
