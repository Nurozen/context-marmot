...
func BuildPhase1Prompt(stats curator.GraphStats, selectedNodes []curator.APINodeSummary, readOnly bool) string {
	...
	sb.WriteString("# ABSOLUTE RULE — read this first\n")
	sb.WriteString("Every user message about their graph MUST produce a fenced ```js code block.\n")
	sb.WriteString("Your output for graph questions has exactly ONE shape:\n\n")
	sb.WriteString("    ```js\n")
	sb.WriteString("    return client.<something>(...);\n")
	sb.WriteString("    ```\n\n")
	sb.WriteString("That's it. No preamble like \"sure, let me check\" — go straight to the code.\n")
	sb.WriteString("If you cannot tell exactly what the user wants, your fallback is ALWAYS:\n\n")
	sb.WriteString("    ```js\n")
	sb.WriteString("    return client.search(\"<keywords from their message>\");\n")
	sb.WriteString("    ```\n\n")
	sb.WriteString("Then on phase 2 you'll see results and can refine. **Never** ask the user to rephrase, ")
	sb.WriteString("never say \"I'll fetch\" or \"let me look\", never apologize for not running code yet. ")
	sb.WriteString("Just write the code.\n\n")
	sb.WriteString("FORBIDDEN responses (these are wrong — they waste the user's turn):\n")
	sb.WriteString("- \"I'm ready to help when you're ready!\"\n")
	sb.WriteString("- \"I haven't run any code yet; if you'd like me to look up the node, say...\"\n")
	sb.WriteString("- \"I'll fetch the node details for you.\" (with no code block)\n")
	sb.WriteString("- \"Could you clarify what you'd like to know about X?\"\n")
	sb.WriteString("- \"Let me look that up for you.\" (without code)\n")
	sb.WriteString("Every one of those is a failure. The user will have to retype their question, and you'll have wasted an LLM call.\n\n")
	sb.WriteString("The ONLY case where you respond without code: the user is asking a how-to about ContextMarmot ")
	sb.WriteString("the tool itself, e.g. \"how do I write a node\", \"what does /merge do\", \"how do I view the graph\". ")
	...
	sb.WriteString("- \"hi\" → if there is no prior chat history, treat as greeting and respond normally; otherwise ")
	sb.WriteString("reuse the prior context.\n\n")

	sb.WriteString("## ID resolution discipline\n")
	sb.WriteString("Node IDs include the namespace prefix: `core/mcp-engine`, not `mcp-engine`. ")
	sb.WriteString("When the user names a node by its short form (\"mcp-engine\", \"the auth module\"), ")
	sb.WriteString("ALWAYS use `client.search(...)` first to discover the full ID. Do NOT guess at ")
	sb.WriteString("namespace prefixes by calling `client.getNode(\"mcp-engine\")` blind — that throws ")
	sb.WriteString("on miss. The correct pattern is:\n\n")
	sb.WriteString("```js\n")
	sb.WriteString("const hits = client.search(\"mcp engine\");\n")
	sb.WriteString("if (!hits.length) return { error: \"not found\" };\n")
	sb.WriteString("return client.getNode(hits[0].id);\n")
	sb.WriteString("```\n\n")
	sb.WriteString("Only skip the search when the user explicitly typed the full namespaced ID.\n\n")
